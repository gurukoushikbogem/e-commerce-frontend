const mainImgs = document.querySelectorAll(".default .main-img img");
const thumbs = document.querySelectorAll(".default .thumb-list div");
const lightboxImgs = document.querySelectorAll(".lightbox .main-img img");
...
const setActiveImage = (index, imgs, thumbs) => {
...
const setupThumbnailClicks = (thumbs, imgs) => {
  thumbs.forEach((thumb, index) => {
    thumb.addEventListener("click", () => setActiveImage(index, imgs, thumbs));
  });
};

const startGallery = () => {
  setupThumbnailClicks(thumbs, mainImgs);
  setupThumbnailClicks(lightboxThumbs, lightboxImgs);

  mainImgs.forEach((img, index) => {
    img.addEventListener("click", () => {
      lightbox.classList.add("active");
      setActiveImage(index, lightboxImgs, lightboxThumbs);
    });
  });

  prevBtn.addEventListener("click", () => {
    const newIndex =
      currentIndex === 0 ? mainImgs.length - 1 : currentIndex - 1;
    setActiveImage(newIndex, lightboxImgs, lightboxThumbs);
  });

  nextBtn.addEventListener("click", () => {
    const newIndex =
      currentIndex === mainImgs.length - 1 ? 0 : currentIndex + 1;
    setActiveImage(newIndex, lightboxImgs, lightboxThumbs);
  });
